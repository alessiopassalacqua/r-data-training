...
Per eseguire il codice scritto nello script:

1. utilizzare il comando `Run` per eseguire la riga su cui è posizionato il cursore
2. utilizzare il comando `Source` per eseguire tutto il codice contenuto nello script
...
Nella sintassi in R esiste un'espressione particolare che viene utilizzata in certi grafici (esempio in `ggplot2`) e in alcune funzioni che creano modelli predittivi (esempio nella funzione `lm()`).
